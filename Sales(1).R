...
confood2 <- read.table("/Users/home/Documents/TAMU/stat608/Data/confood2.txt", header=TRUE)
attach(confood2)
...
#Step 1: Transform the explanatory variable, if desired.
library(car)
bc<-powerTransform(Price)
summary(bc)


#Step 2: Transform the response variable.  (This looks at the residuals!)
preliminary.model <- lm(Sales ~ log(Price))
bc <- powerTransform(preliminary.model)
summary(bc)


#Scatterplot with log transformations, promotions marked:
par(cex.axis=2,cex.lab=2, mar=c(5.1,5.1,2,2),lwd=2)
...
#Notice the normality of the residuals doesn't work so well, as expected!
...
#Plot residuals against y-hat:
preds <-predict(m0)
par(mfrow=c(1,1),cex.axis=2,cex.lab=2, mar=c(5.1,5.1,2,2),lwd=2)
plot(preds, StanRes0, ylab="Stzd Resids", xlab="Predicted Values", pch=Promotion+16,col=Promotion+1)
...
dwt1<-dwt(m1$residuals, max.lag=1)
...
m1 <- lm(log(Sales) ~ log(Price) + Promotion + Week, data=confood2)
n<-length(Week)
rho <- 0.5504  #Estimated in m2 above.
x <- model.matrix(m1)
